...
function onLoad() {
  fetch(
    "https://api.weatherbit.io/v2.0/forecast/daily?ip=auto&days=5&units=I&key=ec69804a082347b592321d0caf2b15cb"
  )
    .then((res) => res.json())
    .then((res) => {
      console.log(res);
      ...
      for (let i = 0; i < highTemp.length; i++) {
        for (let j = 0; j < fiveDayForecastLoad.length; j++) {
          highTemp[i].textContent =
            Math.floor(fiveDayForecastLoad[i].high_temp) + " / ";
      ...
      for (let i = 0; i < narrative.length; i++) {
        for (let j = 0; j < fiveDayForecastLoad.length; j++) {
          ...
          narrative[
            i
          ].textContent = `${windSpeed} MPH winds from the ${windDirection}`;
        }
      }
      ...
      for (let i = 0; i < imgSection.length; i++) {
        for (let j = 0; j < fiveDayForecastLoad.length; j++) {
          imgSection[
            i
          ].style.backgroundImage = `url("icons/icons/${fiveDayForecastLoad[i].weather.icon}.png")`;
        }
      }
      ...
      .then((response) => {
        ...
        for (let i = 0; i < highTemp.length; i++) {
          for (let j = 0; j < fiveDayForecast.length; j++) {
            highTemp[i].textContent =
              Math.floor(fiveDayForecast[i].high_temp) + " /";
          }
        }
